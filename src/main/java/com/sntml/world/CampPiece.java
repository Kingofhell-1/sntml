...
public class CampPiece extends net.minecraft.world.level.levelgen.structure.StructurePiece {
    ...
    public CampPiece(StructureTemplateManager manager, BlockPos pos) {
    ...
    public CampPiece(StructurePieceSerializationContext ctx, CompoundTag tag) {
        super(TYPE, tag);
    ...
    public void postProcess(WorldGenLevel level,
                            ...
                            ChunkPos chunkPos,
                            BlockPos pos) {
        ResourceLocation rl = new ResourceLocation("sntml", "camp");
        ...
        template.get().placeInWorld(level, new BlockPos(originX, originY, originZ), new BlockPos(originX, originY, originZ), settings, random, 2);
    }
}
